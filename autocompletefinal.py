...
dataframe=pd.read_csv('./SIH-Processed.csv');
queries=dataframe['case name']


x=input("Enter query").lower()
tf_idf={}
for word in x.split(' '):
...
print(tf_idf)
if tf_idf!={}:
    sorteddict=dict(sorted(tf_idf.items(), key=lambda item: item[1], reverse=True))
    first_n_values = list(sorteddict.values())[:4]
    first_n_keys=list(sorteddict.keys())[:4]
    for i in range(len(first_n_values)):
